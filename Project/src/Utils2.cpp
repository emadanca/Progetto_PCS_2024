...
Point3D intersectionLines(const Point3D& p1, const Point3D& p2, const Point3D& dir1, const Point3D& dir2)
{
    ...
    if (denominator < tol)
    {
        ...
        cerr << "Le rette sono parallele, non c'è intersezione." << std::endl;
        return {NAN, NAN, NAN};
    }
    Point3D p21 = p2 - p1;
...
void Sottopoligonazione(Fracture& fracture, vector<Trace>& PassingTraces, vector<Trace>& nonPassingTraces, PolygonalMesh& mesh, const double& tol, list<unsigned int>& IDVertices)
{
    ...
    cout << "#### errore ### " << PassingTraces.empty() << endl;
    ...
    // Calcola le differenze tra le coordinate dei punti e ricavo la direttrice della traccia
    Point3D dir = p2-p1;
    ...
    for (unsigned int i = 0; i < fracture.numVertices; i++)
    {
        ...
        unsigned int id_end;  // inizializzo l'ID del vertice di fine del segmento LO DOBBIAMO TROVARE <3
        ...
        if(notParallel)
        {
            ...
            cout << "0 questo è il punto calcolato: " << punto << endl;
            ...
            if (onBoundary)
            {
                ...
                cout << "1 Verifico se il punto esiste già in newPoints." << endl;
                ...
                {
                    cout << "2 Il punto esiste già in newPoints." << endl;
                    ...
                    {
                        cout << "4 Il punto " << punto << " non esiste in fracture.vertices, aggiungo." << endl;
                    ...
                    {
                        cout << "5 Il punto" << punto << " non esiste in mesh.Cell0DCoordinates, aggiungo." << endl;
                        //caso in cui il prolungamento della traccia tagliante tocca una cella 0D già creata
                        newPoints.push_back(punto);  // appendo il punto di intersezione
                        id_newPoints.push_back(id_new_punto);  // push l'id dei nuovi punti
                        ...
                        IDVertices.insert(pos, id_new_punto); // e inseriamo il nuovo punto nella posizione corretta
                    }
                    else
                    {
                        cout << "Il punto è nuovo, lo aggiungo." << endl;
    ...
    // mi baso sul fatto che i so i due punti appena creati
    ...
    //creiamo i vettori contenenti le tracce passanti e non pasanti
    //per il futuro: provare a farlo con le list
    ...
    for (Trace& trace : PassingTraces)  // ciclo sulle tracce
    {
        ...
        // passo 1: ricavo le coodinate degli estremi della traccia
    ...
    // faccio la stessa cosa per la frattura2
...
// - fracture e due serie di tracce (pass e nonPass).
// In pratica, questa funzione divide una fracture in sottopoligoni più piccoli
// e gestisce l'organizzazione delle celle 0D, 1D e 2D all'interno della struttura di dati PolygonalMesh definita in hpp.

PolygonalMesh SubPolygonMesh (const Fracture& fracture, vector<Trace>& PassingTraces, vector<Trace>& nonPassingTraces, const vector<Point3D>& vertices)
{
    PolygonalMesh mesh;
    ...
    for (unsigned int i = 0; i< fracture.numVertices; i++)  // ciclo sul numero di vertici della frattura che passo in input
    {
    ...
    if (PassingTraces.size() + nonPassingTraces.size() == 0)
    {
        ...
        for (unsigned int v = 0; v < fracture.numVertices; v++)// ciclo sul numero di LATI
        {
